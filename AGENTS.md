...
- `configure-locale` writes `LC_COLLATE=C` and `LC_TIME=en_DK.UTF-8` into
  `/etc/locale.conf` (byte-order sort, ISO timestamps) and leaves the
  other `LC_*` keys alone. Needs `locales-en` for `en_DK`.
