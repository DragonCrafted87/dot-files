...
2. Scripts MUST be executable and safe to run with no arguments.
3. Scripts MUST NOT source each other. Orchestration is process calls
   from `display-switch.sh` (or idle wrappers) only.
4. Hostname-specific constants MUST NOT be hardcoded in scripts when a
   `hosts.d`, `monitors.d`, or `audio.d` key can hold them.
5. Conf files that Hyprland does not parse (`hosts.d`, `audio.d`, and
   `MATCH_DESC=` lines) MUST be `KEY=value` lines with `#` comments.
6. Scripts MUST treat missing optional tools (`pactl`, `inotifywait`,
   `notify`) as skippable: print a message, do not abort the profile
   apply.
7. User-facing commands MUST print a one-line `Usage:` on unknown flags
   (`-h` / `--help` / `help`).
...
2. `display-audio.sh` MUST NOT write `display-profile`.
3. The saved profile value MUST be a single trimmed token (`desk`,
   `theater`, `workshare`, or another `monitors.d/<host>-<name>.conf`
   suffix).
4. If the state file is missing, restore MUST use `desk` when that
...
2. Detection MUST use `hyprctl monitors` identity (description / make /
   model / serial) against each single profile's `MATCH_DESC`
   (case-insensitive substring).
3. If no `MATCH_DESC` hits, `DEFAULT_SINGLE_PROFILE` MUST be used when
   set; otherwise `single` MUST fail with a message.
4. Watch MUST daemonize with `setsid` and record the session leader PID
   in `display-switch.pid`.
5. Watch MUST block on `inotifywait` for
   `/sys/class/drm/<SWITCH_DRM>/{status,edid}` (or `card*-<SWITCH_PORT>`
   if `SWITCH_DRM` is unset).
6. After an inotify event, watch MUST wait until the port has a live
   mode, then compare a status+EDID fingerprint, then apply only if the
   detected single profile changed.
7. Disconnect / missing EDID MUST NOT flip the saved profile to desk.
8. Watch MUST exit when the saved profile is no longer in
   `SINGLE_PROFILES`, or when `stop` / `desk` kills the process group.
9. `inotify-tools` missing: `single` MUST still apply the current panel
...
2. Single-output MUST be one bind (`SUPER+SHIFT+S` →
   `display-switch.sh single`), not separate theater and workshare binds.
3. Binds MUST call `display-switch.sh`, not `display-profile.sh` or
...
2. Idle MUST NOT change the saved profile or the audio sink.
3. On runewyrm, idle-off MUST save the workspace map, DPMS the desk
   ports if they are part of the active layout, then disable
   `IDLE_MONITOR` so HDMI cannot wake itself.
4. Idle-on MUST reapply the active monitor conf and restore workspaces.
...
1. Desk on runewyrm MUST select the onboard S/PDIF soundbar
   ...
   speaker.
2. Theater MUST use the GPU HDMI sink when present, with the stereo
   HDMI profile as fallback (`audio.d/runewyrm-stereo.conf`).
3. Workshare MUST use the same soundbar policy as desk.
4. Sink matching MUST be data from `audio.d`, not hostname `case`
...
2. Unknown Hyprland connector names are ignored; explicit modes on
   shared names are not. Keep those modes out of the shared file.
3. `hyprctl keyword monitor` specs MUST strip a trailing `Hz` on the
   refresh field (`2560x1440@143.91`, not `@143.91Hz`).
4. Apply and idle MUST take `apply.lock` so overlapping binds and
