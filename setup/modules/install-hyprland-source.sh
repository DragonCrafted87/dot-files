#!/usr/bin/env bash
# Build the pinned Hyprland tag plus the hypr* ecosystem into an isolated
...
# Ecosystem tags from versions.conf. Override a single tag in the
...
# One argv per flag so CFLAGS with spaces are not split into fake
...
fill_cmake_config_flags() {
    ...
    if [[ -n "${AR:-}" ]]; then
        cmake_config_flags+=("-DCMAKE_AR=${AR}")
    fi
    if [[ -n "${RANLIB:-}" ]]; then
        cmake_config_flags+=("-DCMAKE_RANLIB=${RANLIB}")
    fi
}
...
install_session_files() {
    ...
    sudo sed -i -E \
        "s|source 0\\.56\\.2|source ${HYPRLAND_SOURCE_VERSION}|g; s|v0\\.56\\.2|${HYPRLAND_TAG}|g" \
        "${PREFIX}/share/wayland-sessions/${desktop_name}" \
        "${WAYLAND_SESSION_DIR}/${desktop_name}" \
        "${LY_CUSTOM_DIR}/${desktop_name}"
}
